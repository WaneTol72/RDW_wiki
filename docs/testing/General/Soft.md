...
- Для оптимизации работы с образом RDW_TEST_OPTIMIZED:
	...
	- Работа с ПЗ (terminal, авто-ввод логина/пароля, работа с мультикастом).
	...
	- На выключение *caps lock*, *num lock*, переключения языка на eng и другое.
...
- `Veyon Master` - Для мониторинга ПК (только для Desktop Case: улучшает производительность, уменьшает рассинхрон)
